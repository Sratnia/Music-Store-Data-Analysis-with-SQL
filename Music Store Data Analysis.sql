...
SELECT customer_id, SUM(total) as total_spent FROM invoice GROUP BY customer_id ORDER BY total_spent DESC LIMIT 1;
...
select a.artist_id, a.name, count(a.artist_id) as track_count from track t
...
where milliseconds > (select avg(milliseconds) as avg_song_lgth from track)
